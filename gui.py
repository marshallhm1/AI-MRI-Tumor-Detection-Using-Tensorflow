...
def browse_file():
    ...
    if file_path:
        ...
        if detected_tumor_type:
            update_display(file_path, detected_tumor_type)  # Pass file path here
...
def update_display(image_path, tumor_type):
    ...
    # Create a Label with grey background
    ...
    image_label.grid(row=0, column=0, padx=20, pady=20, columnspan=2,
                     sticky="nsew")  # Centered with columnspan and sticky
...
def show_about():
    about_text = ("""This app detects accepts image input in the form of a brain MRI, and can detect the presence of 
                     3 different kinds of tumors, Glioblastoma, Meningioma, and Pituitary, as well as detecting images 
                     with no tumor. Developed using Python, Tensorflow, Xception for machine learning, 
                    TKinter for the visual app. Created by Marshall Morgan for HackRice13. Not medical advice,
                    and should not replace a doctor.""")
...
if __name__ == "__main__":
    ...
    root.geometry("1280x800")  # Change the size to your preferred dimensions

    # Set the background color
    root.configure(bg='grey')  # You can change 'grey' to your desired color
    ...
    # Place the Browse button at the bottom
